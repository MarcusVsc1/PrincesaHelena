 ...
 * @default 100
 *
 * @param Cooldown Persistent Skills
 * @parent ---Cooldown---
 * @type skill[]
 * @desc The list of skill IDs that will have their cooldowns persist after battles.
 * @default []
 *
 * @help
 * ============================================================================
...
Yanfly.Parameters = PluginManager.parameters('YEP_X_SkillCooldowns - Revised');
...
Yanfly.SetupParameters = function() {
  ...
  Yanfly.Param.CDPersistentSkills = JSON.parse(Yanfly.Parameters['Cooldown Persistent Skills'] || '[]').map(Number)
...
Game_BattlerBase.prototype.endBattleCooldowns = function() {
    ...
    data = Yanfly.Param.CDPersistentSkills;

    for (var [key, cooldown] of Object.entries(this._cooldownTurns)) {
    	if(!data.includes(Number(key))) {
    		this._cooldownTurns[key] = 0;	
    	}
